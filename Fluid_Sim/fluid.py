...
# ------ READ INITIAL STATE ----------------
def initial_state(in_state):
    file = open(in_state, "r")
    contents = file.read()
    in_state_dict = json.loads(contents)
    return in_state_dict


# ------- PROCESS VELOCITY FROM JSON --------------
def velocity_behaviour(vel, frame):
    #rx means rotation in x
    if vel[-1] == "rotx": velocity = [vel[0], vel[1]*np.cos(.4 * frame)]
    #rx means rotation in y
    elif vel[-1] == "roty": velocity = [vel[0]*np.sin(.4 * frame), vel[1]]
    #circular rotation
    elif vel[-1] == "rotcirc": velocity = [vel[0]*np.sin(.4 * frame), vel[1]*np.cos(.4*frame)]
    #linear velocity
    else: velocity = [vel[0], vel[1]]
    return velocity
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        # read configuration
        in_state = initial_state(sys.argv[1])
        ...
        try:
            ...
            for keys in in_state:
                print(keys, in_state[keys])

            def update_im(i):
                # We add new density creators in here
                # inst.density[14:17, 14:17] += 100  # add density into a 3*3 square
                # inst.density[10:13, 10:17] = 0
                for density in in_state["density_list"]:
                    inst.density[density["pos1"][0]:density["pos2"][0],
                                density["pos1"][1]:density["pos2"][1]] += density["value"]

                # We add velocity vector values in here
                # inst.velo[20, 20] = [-2, -2]
                for velocity in in_state["velocity_list"]:
                    vel = velocity_behaviour(velocity["direction"], i)
                    inst.velo[velocity["pos"][0], velocity["pos"][1]] = vel
            ...
            im = plt.imshow(inst.density, vmax=100,
                            ...
                            cmap=in_state["colors"]["cmap"])

            # plot vector field
            q = plt.quiver(inst.velo[:, :, 1], inst.velo[:, :, 0],
                        scale=10, angles='xy', color=in_state["colors"]["quiver"])

            anim = animation.FuncAnimation(fig, update_im, interval=1, frames=300)
            # anim.save("movie2.mp4", fps=30, extra_args=['-vcodec', 'libx264'])
            Writer = animation.writers['pillow']
            writer = Writer(fps=30, bitrate=2500)
            anim.save("Fluid_Sim\\fluid_simulation.gif", writer)
            plt.show()
        ...
        print("No json detected. Please try again.\n")
        exit(0)
